...
class test_Graph_Service__Fast_API__client(TestCase):
    ...
    def setUpClass(cls):
        with setup__graph_service_fast_api_test_objs() as _:
            ...
            cls.client.headers[TEST_API_KEY__NAME] = ''                                  # Clear auth for initial tests

    def test__init__(self):                                                              # Test initialization of test objects
    ...
    def test__client__auth(self):                                                        # Test authentication flow
        path                = '/info/health'
    ...
    def test__info_routes(self):                                                         # Test info/health endpoints
        auth_key_name  = get_env(ENV_VAR__FAST_API__AUTH__API_KEY__NAME )
        auth_key_value = get_env(ENV_VAR__FAST_API__AUTH__API_KEY__VALUE)
        headers        = {auth_key_name: auth_key_value}

        response = self.client.get(url='/info/health', headers=headers)
