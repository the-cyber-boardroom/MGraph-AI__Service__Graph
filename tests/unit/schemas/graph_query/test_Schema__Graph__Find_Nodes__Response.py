from types                                                                                  import NoneType
from unittest                                                                               import TestCase
from osbot_utils.testing.__                                                                 import __
from osbot_utils.type_safe.Type_Safe                                                        import Type_Safe
from osbot_utils.type_safe.type_safe_core.collections.Type_Safe__List                       import Type_Safe__List
from osbot_utils.type_safe.primitives.core.Safe_UInt                                        import Safe_UInt
from osbot_utils.type_safe.primitives.domains.identifiers.Obj_Id                            import Obj_Id
from osbot_utils.type_safe.primitives.domains.identifiers.Random_Guid                       import Random_Guid
from osbot_utils.type_safe.primitives.domains.identifiers.safe_str.Safe_Str__Id             import Safe_Str__Id
from osbot_utils.utils.Objects                                                              import base_classes
from mgraph_ai_service_cache_client.schemas.cache.Cache_Id                                  import Cache_Id
from mgraph_ai_service_graph.schemas.graph_ref.Graph_Id                                     import Graph_Id
from mgraph_ai_service_graph.schemas.graph_ref.Schema__Graph__Ref                           import Schema__Graph__Ref, GRAPH_REF__DEFAULT_NAMESPACE
from mgraph_ai_service_graph.schemas.graph_query.Schema__Graph__Find_Nodes__Response        import Schema__Graph__Find_Nodes__Response


class test_Schema__Graph__Find_Nodes__Response(TestCase):

    # ═══════════════════════════════════════════════════════════════════════════════
    # Initialization Tests
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__init__(self):                                                                 # Test auto-initialization
        with Schema__Graph__Find_Nodes__Response() as _:
            assert type(_)             is Schema__Graph__Find_Nodes__Response
            assert base_classes(_)     == [Type_Safe, object]
            assert _.graph_ref         is None                                              # Optional graph_ref
            assert type(_.node_ids)    is Type_Safe__List                                   # Type_Safe__List not raw list
            assert type(_.total_found) is Safe_UInt
            assert type(_.has_more)    is bool
            assert _.has_more          is False

    def test__init__field_types(self):                                                      # Test field types at initialization
        with Schema__Graph__Find_Nodes__Response() as _:
            assert type(_.graph_ref)   is NoneType
            assert type(_.node_ids)    is Type_Safe__List
            assert type(_.total_found) is Safe_UInt
            assert type(_.has_more)    is bool

    def test__init__obj_comparison(self):                                                   # Test .obj() with default values
        with Schema__Graph__Find_Nodes__Response() as _:
            assert _.obj() == __(graph_ref   = None ,
                                 node_ids    = []   ,
                                 total_found = 0    ,
                                 has_more    = False)

    # ═══════════════════════════════════════════════════════════════════════════════
    # Value Assignment Tests - Success Cases
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__with_graph_ref__results(self):                                                # Test with results found
        graph_id  = Graph_Id(Obj_Id())
        cache_id  = Cache_Id(Random_Guid())
        node_ids  = [Obj_Id(), Obj_Id(), Obj_Id()]
        graph_ref = Schema__Graph__Ref(graph_id  = graph_id       ,
                                       cache_id  = cache_id       ,
                                       namespace = 'test-namespace')

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 100      ,
                                                 has_more    = True     ) as _:
            assert _.graph_ref.graph_id  == graph_id
            assert _.graph_ref.cache_id  == cache_id
            assert _.graph_ref.namespace == 'test-namespace'
            assert len(_.node_ids)       == 3
            assert _.total_found         == 100
            assert _.has_more            is True

    def test__with_cache_id_lookup(self):                                                   # Test response when graph was found by cache_id
        cache_id  = Cache_Id(Random_Guid())
        graph_id  = Graph_Id(Obj_Id())
        node_ids  = [Obj_Id(), Obj_Id()]
        graph_ref = Schema__Graph__Ref(cache_id  = cache_id ,
                                       graph_id  = graph_id ,                               # Resolved after lookup
                                       namespace = 'cache-ns')

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 2        ,
                                                 has_more    = False    ) as _:
            assert _.graph_ref.cache_id == cache_id
            assert _.graph_ref.graph_id == graph_id
            assert len(_.node_ids)      == 2
            assert _.has_more           is False

    # ═══════════════════════════════════════════════════════════════════════════════
    # Empty Results Tests
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__empty_results(self):                                                          # Test with no results
        graph_ref = Schema__Graph__Ref(graph_id = Graph_Id(Obj_Id()))

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = []       ,
                                                 total_found = 0        ,
                                                 has_more    = False    ) as _:
            assert len(_.node_ids) == 0
            assert _.total_found   == 0
            assert _.has_more      is False

    # ═══════════════════════════════════════════════════════════════════════════════
    # Pagination Tests
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__pagination_scenario__first_page(self):                                        # Test first page with more results
        graph_ref = Schema__Graph__Ref(graph_id = Graph_Id(Obj_Id()))
        node_ids  = [Obj_Id() for _ in range(10)]

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 50       ,
                                                 has_more    = True     ) as _:
            assert len(_.node_ids) == 10
            assert _.total_found   == 50
            assert _.has_more      is True                                                  # More results available

    def test__pagination_scenario__last_page(self):                                         # Test last page with no more results
        graph_ref = Schema__Graph__Ref(graph_id = Graph_Id(Obj_Id()))
        node_ids  = [Obj_Id() for _ in range(5)]

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 45       ,
                                                 has_more    = False    ) as _:
            assert len(_.node_ids) == 5
            assert _.total_found   == 45
            assert _.has_more      is False                                                 # No more results

    def test__pagination_scenario__exact_page(self):                                        # Test exact page size
        graph_ref = Schema__Graph__Ref(graph_id = Graph_Id(Obj_Id()))
        node_ids  = [Obj_Id() for _ in range(10)]

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 10       ,
                                                 has_more    = False    ) as _:
            assert len(_.node_ids) == 10
            assert _.total_found   == 10
            assert _.has_more      is False

    # ═══════════════════════════════════════════════════════════════════════════════
    # Type Validation Tests
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__graph_ref_field_types(self):                                                  # Test types within graph_ref
        graph_id  = Graph_Id(Obj_Id())
        cache_id  = Cache_Id(Random_Guid())
        node_ids  = [Obj_Id()]
        graph_ref = Schema__Graph__Ref(graph_id  = graph_id ,
                                       cache_id  = cache_id ,
                                       namespace = 'type-ns')

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 1        ,
                                                 has_more    = False    ) as _:
            assert type(_.graph_ref)           is Schema__Graph__Ref
            assert type(_.graph_ref.graph_id)  is Graph_Id
            assert type(_.graph_ref.cache_id)  is Cache_Id
            assert type(_.graph_ref.namespace) is Safe_Str__Id
            assert type(_.node_ids)            is Type_Safe__List
            assert type(_.total_found)         is Safe_UInt
            assert type(_.has_more)            is bool

    # ═══════════════════════════════════════════════════════════════════════════════
    # Serialization Tests
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__serialization_round_trip__minimal(self):                                      # Test JSON round-trip with minimal data
        with Schema__Graph__Find_Nodes__Response() as original:
            json_data = original.json()

            with Schema__Graph__Find_Nodes__Response.from_json(json_data) as restored:
                assert restored.graph_ref   == original.graph_ref
                assert restored.total_found == original.total_found
                assert restored.has_more    == original.has_more

    def test__serialization_round_trip__complete(self):                                     # Test JSON round-trip with all fields
        graph_ref = Schema__Graph__Ref(graph_id  = Graph_Id(Obj_Id())    ,
                                       cache_id  = Cache_Id(Random_Guid()),
                                       namespace = 'serial-ns'           )
        node_ids  = [Obj_Id(), Obj_Id()]

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 25       ,
                                                 has_more    = True     ) as original:
            json_data = original.json()

            with Schema__Graph__Find_Nodes__Response.from_json(json_data) as restored:
                assert restored.graph_ref.graph_id  == original.graph_ref.graph_id
                assert restored.graph_ref.cache_id  == original.graph_ref.cache_id
                assert restored.graph_ref.namespace == original.graph_ref.namespace
                assert restored.total_found         == original.total_found
                assert restored.has_more            == original.has_more

    def test__serialization_preserves_types(self):                                          # Test that types are preserved
        graph_ref = Schema__Graph__Ref(graph_id  = Graph_Id(Obj_Id()),
                                       namespace = 'type-test'       )
        node_ids  = [Obj_Id()]

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 1        ) as original:
            json_data = original.json()

            with Schema__Graph__Find_Nodes__Response.from_json(json_data) as restored:
                assert type(restored.graph_ref)           is Schema__Graph__Ref
                assert type(restored.graph_ref.graph_id)  is Graph_Id
                assert type(restored.graph_ref.namespace) is Safe_Str__Id
                assert type(restored.node_ids)            is Type_Safe__List
                assert type(restored.total_found)         is Safe_UInt

    # ═══════════════════════════════════════════════════════════════════════════════
    # Edge Cases
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__default_namespace(self):                                                      # Test default namespace from Schema__Graph__Ref
        graph_ref = Schema__Graph__Ref(graph_id = Graph_Id(Obj_Id()))                       # Uses default namespace

        with Schema__Graph__Find_Nodes__Response(graph_ref = graph_ref) as _:
            assert _.graph_ref.namespace == GRAPH_REF__DEFAULT_NAMESPACE

    def test__large_result_set(self):                                                       # Test with large number of results
        graph_ref = Schema__Graph__Ref(graph_id = Graph_Id(Obj_Id()))
        node_ids  = [Obj_Id() for _ in range(100)]

        with Schema__Graph__Find_Nodes__Response(graph_ref   = graph_ref,
                                                 node_ids    = node_ids ,
                                                 total_found = 10000    ,
                                                 has_more    = True     ) as _:
            assert len(_.node_ids) == 100
            assert _.total_found   == 10000
            assert _.has_more      is True